...
class ModelNile:
    ...
    def __init__(self, directory, growth_coef, irr_lambda_std, evap_increase_coef, evap_lambda_mean, evap_lambda_std):
        """
        ...
        for name in self.catchment_names:
            new_catchment = Catchment(name, directory)
            self.catchments[name] = new_catchment

        self.irr_districts = dict()
        for name in self.irr_district_names:
            new_irr_district = IrrigationDistrict(name, growth_coef, irr_lambda_std)
        ...
        for name in self.reservoir_names:
            new_reservoir = Reservoir(name, evap_increase_coef, evap_lambda_mean, evap_lambda_std)
    ...
    def evaluate(self, parameter_vector, trace = 101):  # , uncertainty_dict
    ...
    def reset_parameters(self, trace=101):
        if trace == 101:
            trace = np.random.rand(0, 100)
        for reservoir in self.reservoirs.values():
        ...
        for catchment in self.catchments.values():
            catchment.reset(trace)
