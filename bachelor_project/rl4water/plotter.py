...
def normalize_objs(df, directions):
    ...
    for i, col in enumerate(df.columns):
        ...
        working_df[col] = (df[col] - worst) / (best - worst - 1e-9)
...
def parallel_plots_many_policies(
    ...
    units=["BCM/year", "%", "BCM/year", "TWh/year"],
    ...
    for i in desirability_couples:
        uds.append(str(round(i[0], 3)))
        ds.append(str(round(i[1], 3)))
    ...
    for i, solution_index in enumerate(solution_indices):
        ...
        to_be_added = norm_df.loc[solution_index, :].copy()
        norm_df = pd.concat([norm_df, pd.DataFrame(to_be_added).T])
...
def parallel_plots_many_policies_two(
    obj_df,
    obj_df2,
    solution_indices=[],
    solution_names=[],
    ...
    names = list(obj_df.columns)
    names2 = list(obj_df2.columns)

    objectives_df = obj_df.copy()
    objectives_df2 = obj_df2.copy()
    objectives_df.egypt_low_had = 100 * (objectives_df.egypt_low_had)
    objectives_df2.egypt_low_had = 100 * (objectives_df2.egypt_low_had)

    norm_df, desirability_couples = normalize_objs(objectives_df, directions)
    norm_df2, desirability_couples2 = normalize_objs(objectives_df2, directions)
    ...
    uds2 = []  # undesired
    ds2 = []  # desired
    for i in desirability_couples2:
        uds2.append(str(round(i[0], 1)))
        ds2.append(str(round(i[1], 1)))

    norm_df["Name"] = "Hurst Solutions"
    norm_df2["Name"] = "Baseline Solutions"
    ...
    handles, labels = plt.gca().get_legend_handles_labels()
    handles_dict = dict(zip(labels, handles))
    labels = ["Hurst Solutions", "Baseline Solutions"]
